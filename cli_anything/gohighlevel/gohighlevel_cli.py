...
def _load_recipe(module_name: str):
    """Import a recipe module from the repo's recipes/ folder (one source of truth).

    Recipes are dependency-free scripts that also back some CLI subcommands, so the
    same logic runs whether invoked as `python3 recipes/<x>.py` or via the CLI/MCP.
    """
    import importlib
    from pathlib import Path

    recipes_dir = Path(__file__).resolve().parents[2] / "recipes"
    if not (recipes_dir / f"{module_name}.py").exists():
        raise click.ClickException(
            f"Recipe '{module_name}' not found at {recipes_dir}. Recipes ship in the "
            "repo's recipes/ folder — run from a source checkout (pip install -e .)."
        )
    if str(recipes_dir) not in sys.path:
...
@calendars.command("convert-to-round-robin")
@click.option("--calendar", default=None,
              help="Source calendar id or name (default: the single active event calendar)")
@click.option("--user", "users", multiple=True, required=True, metavar="NAME|EMAIL|ID",
              help="Team member to assign (repeatable; first = primary)")
@click.option("--distribution", type=click.Choice(["optimize-availability", "equal"]),
              default="optimize-availability", help="Round-robin distribution")
@click.option("--name", default=None, help="Name for the new calendar (default: same as source)")
@click.option("--keep-source", is_flag=True, help="Leave the source calendar active")
@click.option("--dry-run", is_flag=True, help="Preview only; make no changes")
@click.pass_context
def calendars_convert_rr(ctx, calendar, users, distribution, name, keep_source, dry_run):
    """Recreate an EVENT calendar as a ROUND ROBIN calendar.

    GHL can't change a calendar's type in place, so this copies all settings into a
    new round_robin calendar, assigns the team member(s), and deactivates the source.
    Needs the agency's Firebase token (internal API), same as workflow building.
    """
    import os

    recipe = _load_recipe("convert_calendar_to_round_robin")
    lib = _load_recipe("_ghl_recipe_lib")
    if ctx.obj.get("location_id"):
        os.environ["GHL_LOCATION_ID"] = ctx.obj["location_id"]
    try:
        prof = lib.ensure_creds(None)  # creds already exported by ghl-as
        result = recipe.convert(prof, calendar=calendar, users=list(users),
                                distribution=distribution, name=name,
                                deactivate_source=not keep_source, dry_run=dry_run)
        if ctx.obj.get("json"):
            click.echo(json.dumps(result, indent=2, default=str))
        else:
            recipe.print_result(result)
    except Exception as e:
        _handle_error(e)
